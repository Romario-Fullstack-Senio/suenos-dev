...
@Injectable()
export class RegistrarCursoCompradoHandler {
  private readonly logger = new Logger(RegistrarCursoCompradoHandler.name);
  ...
  async handle(event: CursoCompradoPayload): Promise<void> {
    ...
    const insigniasNuevas = perfil.registrarCursoComprado();
